import asyncio
import os
from telegram import Update, InlineKeyboardButton, InlineKeyboardMarkup
from telegram.ext import Application, CommandHandler, MessageHandler, CallbackQueryHandler, filters, ContextTypes

BOT_TOKEN = os.environ["BOT_TOKEN"]
OWNER_ID = int(os.environ["OWNER_ID"])

allowed_users = set()
user_log = {}  # {user_id: chat_id}
waiting_for_sender = {}   # {user_id: target}
waiting_for_custom = {}   # {user_id: (sender, target)} — custom text wait
active_fights = {}        # {user_id: True}
fight_data = {}           # {user_id: (sender, target)}

# =============================================
# OWNER COMMANDS
# =============================================

async def allow(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != OWNER_ID:
        return
    user_id = None
    name = None
    if update.message.reply_to_message:
        user_id = update.message.reply_to_message.from_user.id
        name = update.message.reply_to_message.from_user.first_name
    elif update.message.entities:
        for entity in update.message.entities:
            if entity.type == "text_mention":
                user_id = entity.user.id
                name = entity.user.first_name
                break
    if user_id:
        allowed_users.add(user_id)
        await update.message.reply_text(f"✅ {name} BKL AAGYA PERMISSION!")
    else:
        await update.message.reply_text("❌ GAWAR KISI KO MANTION TO KAR!!!")

async def remove(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != OWNER_ID:
        return
    user_id = None
    name = None
    if update.message.reply_to_message:
        user_id = update.message.reply_to_message.from_user.id
        name = update.message.reply_to_message.from_user.first_name
    elif update.message.entities:
        for entity in update.message.entities:
            if entity.type == "text_mention":
                user_id = entity.user.id
                name = entity.user.first_name
                break
    if user_id and user_id in allowed_users:
        allowed_users.remove(user_id)
        await update.message.reply_text(f"❌ {name} JA PHIR SE PERMISSION LEKE AA! @ruchika_owns")
    else:
        await update.message.reply_text("TUJE TO KABHI DEKHA NAI JA PERMISSION LEKE AA @ruchika_owns")

async def logs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != OWNER_ID:
        return
    if not user_log:
        await update.message.reply_text("Abhi kisi ne use nahi kiya.")
        return
    msg = "📋 Fight use karne wale:\n\n"
    for uid, cid in user_log.items():
        msg += f"• User ID: `{uid}` | Chat ID: `{cid}`\n"
    await update.message.reply_text(msg, parse_mode="Markdown")

# =============================================
# STOP COMMAND
# =============================================

async def stop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if user_id in active_fights:
        active_fights.pop(user_id)
        await update.message.reply_text("\U0001F6D1 CHUDAI KHATAM!!!!! KHUSH HO JA CHUDAI KHATAM KAR DI TUNE")
    else:
        await update.message.reply_text("KOI CHUDEGA?")

# =============================================
# FIGHT COMMAND
# =============================================

async def fight(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    chat_id = update.effective_chat.id

    if user_id != OWNER_ID and user_id not in allowed_users:
        await update.message.reply_text("\u26D4NIKAL PERMISSION LEKE AA PAHLE @ruchika_owns")
        return
    if user_id in active_fights:
        await update.message.reply_text("\u26A0\uFE0F PAHLE STOP TO KAR BKL!")
        return

    # Chat ID save karo
    user_log[user_id] = chat_id

    target = None
    if context.args:
        target = " ".join(context.args)
    if not target:
        await update.message.reply_text("❌ Use: /fight @TargetName")
        return

    waiting_for_sender[user_id] = target
    await update.message.reply_text(
        f"\u2694\uFE0F Target: *{target}*\n\nNAME BOL JALDI SE APNA:",
        parse_mode="Markdown"
    )

# =============================================
# SENDER NAAM LENA → MENU DIKHAO
# =============================================

async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id

    # Custom text wait
    if user_id in waiting_for_custom:
        sender, target = waiting_for_custom.pop(user_id)
        custom_text = update.message.text.strip()
        active_fights[user_id] = True
        asyncio.create_task(run_custom_fight(update, context, custom_text, user_id))
        return

    if user_id not in waiting_for_sender:
        await update.message.reply_text("JA @ruchika_owns SE PERMISSION LEKE AA")
        return

    sender = update.message.text.strip()
    target = waiting_for_sender.pop(user_id)
    fight_data[user_id] = (sender, target)

    # Menu dikhao
    keyboard = [
        [InlineKeyboardButton("1️⃣ Short Text", callback_data=f"short|{user_id}")],
        [InlineKeyboardButton("2️⃣ Long Text", callback_data=f"long|{user_id}")],
        [InlineKeyboardButton("3️⃣ Khud Dega Text", callback_data=f"custom|{user_id}")],
        [InlineKeyboardButton("4️⃣ Fun Text", callback_data=f"fun|{user_id}")],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text(
        f"⚔️ *{sender}* vs *{target}*\n\nKaunsa spam karna hai?",
        reply_markup=reply_markup,
        parse_mode="Markdown"
    )

# =============================================
# BUTTON PRESS HANDLE
# =============================================

async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()

    data = query.data.split("|")
    spam_type = data[0]
    user_id = int(data[1])

    if user_id not in fight_data:
        await query.edit_message_text("❌ Fight data nahi mila, dobara /fight karo.")
        return

    sender, target = fight_data[user_id]

    if spam_type == "custom":
        waiting_for_custom[user_id] = (sender, target)
        await query.edit_message_text("✍️ Apna custom text bhejo — woh bar bar spam hoga:")
        return

    active_fights[user_id] = True
    await query.edit_message_text(f"🔥 Spam shuru! /stop se band karo.")
    asyncio.create_task(run_fight(update, context, sender, target, user_id, spam_type))

# =============================================
# FIGHT MESSAGES
# =============================================

async def run_fight(update, context, sender, target, user_id, spam_type):
    blood = "\U0001FA78"
    fire = "\U0001F525"
    hot = "\U0001F975"
    lol = "\U0001F923"
    puke = "\U0001F922\U0001F92E"

    if spam_type == "short":
        messages = [
            f"{target} TERI MAA KA PEROID {blood*5}",
            f"{sender} PAPA ON FIRE {fire*5}",
            f"{target} CHHOTA SA DIMAG {lol*5}",
            f"{sender} KING HAI {fire*5}",
            f"{target} NIKAL YAHAN SE {puke}",
        ]
    elif spam_type == "long":
        messages = [
            f"created by RACHIT X RUCHIKA",
            f"{target} TERI MAA KA PEROID {blood*10}",
            f"{sender} PAPA ON FIRE {fire*10}",
            f"{sender} fuck by {target} {hot*10}",
            f"{target} TERI MA KE NUDES KO VPS EDIT BANA DU? {lol*10}",
            f"{target} TERI MAA KI CHUT SE BADBU ARHI HE CHUT KESE MARU USKI {puke}",
            f"{target} TERI MAA KA PEROID {blood*10}",
            f"{sender} PAPA ON FIRE {fire*10}",
            f"{sender} fuck by {target} {hot*10}",
            f"{target} TERI MA KE NUDES KO VPS EDIT BANA DU? {lol*10}",
            f"{target} TERI MAA KI CHUT SE BADBU ARHI HE CHUT KESE MARU USKI {puke}",
            f"{target} TERI MAA KA PEROID {blood*10}",
            f"{sender} PAPA ON FIRE {fire*10}",
            f"{sender} fuck by {target} {hot*10}",
            f"{target} TERI MA KE NUDES KO VPS EDIT BANA DU? {lol*10}",
            f"{target} TERI MAA KI CHUT SE BADBU ARHI HE CHUT KESE MARU USKI {puke}",
            f"created by RACHIT X RUCHIKA",
        ]
    elif spam_type == "fun":
        messages = [
            f"{target} tu ek baar aur bol {lol*5}",
            f"{sender} ne {target} ko phir se haraaya {fire*5}",
            f"{target} rona mat shuru ho {lol*5}",
            f"{sender} undefeated champion {fire*5}",
            f"{target} ghar ja beta {puke}",
        ]
    else:
        messages = [f"{target} TERI MAA KA PEROID {blood*10}"]

    while user_id in active_fights:
        msg = "\n\n\n\n".join(messages)
        try:
            chat_id = update.callback_query.message.chat_id
            await context.bot.send_message(chat_id=chat_id, text=msg)
        except Exception:
            break
        await asyncio.sleep(0.02)

async def run_custom_fight(update, context, custom_text, user_id):
    while user_id in active_fights:
        try:
            await update.message.reply_text(custom_text)
        except Exception:
            break
        await asyncio.sleep(0.02)

# =============================================
# UNKNOWN COMMAND
# =============================================

async def unknown(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if user_id in waiting_for_sender or user_id in waiting_for_custom:
        return
    await update.message.reply_text("JA @ruchika_owns SE PERMISSION LEKE AA")

# =============================================
# MAIN
# =============================================

def main():
    app = Application.builder().token(BOT_TOKEN).build()
    app.add_handler(CommandHandler("allow", allow))
    app.add_handler(CommandHandler("remove", remove))
    app.add_handler(CommandHandler("logs", logs))
    app.add_handler(CommandHandler("fight", fight))
    app.add_handler(CommandHandler("stop", stop))
    app.add_handler(CallbackQueryHandler(button_handler))
    app.add_handler(MessageHandler(filters.COMMAND, unknown))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_message))
    print("✅ Fight Bot chal raha hai...")
    app.run_polling()

if __name__ == "__main__":
    main()
